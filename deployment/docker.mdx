...
<Tip>
  Add a `.dockerignore` to exclude `.env`, `tests/`, and `poetry.lock` from the build context for faster builds and to avoid accidentally leaking local secrets into the image. The repository already ships a `.dockerignore` that excludes `.env`, `.env.test`, `.venv`, `__pycache__`, and cache directories.
</Tip>
